...
def test_검증기_쓰기_올바른_입력_성공(validator):
    assert validator.validate_test(CORRECT_WRITE_SENTENCE) == True
...
def test_검증기_쓰기_잘못된_입력_길이(wrong_input, validator):
    assert validator.validate_test(wrong_input) == False
    
def test_검증기_읽기_올바른_입력_성공(validator):
    assert validator.validate_test(CORRECT_READ_SENTENCE) == True
...
def test_검증기_앞뒤_공백_문자(validator):
    ...
    for wrong_sentence in wrong_sentences:
        assert validator.validate_test(wrong_sentence) == True
...
def test_검증기_대소문자_성공(validator):
    ...
    for correct_sentence in correct_sentences:
        assert validator.validate_test(correct_sentence) == True
